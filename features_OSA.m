...
for n1=1:length(all)
	...
	st_r=[length(find(data1_label==1))/length(data1_label),length(find(data1_label==2))/length(data1_label),length(find(data1_label==3))/length(data1_label),...
        length(find(data1_label==4))/length(data1_label),length(find(data1_label==5))/length(data1_label)];
	...
	%save(sprintf('/user/chae2089/CWpaper/FN_REVISION/feature_for_2nd_model/eeg_combined_spo2_str/subject%d.mat',all(n1)), 'nrem_rem_all_comb_spo2');
	%save(sprintf('/user/chae2089/CWpaper/FN_REVISION/feature_for_2nd_model/eeg_alone_str/subject%d_rdi.mat',all(n1)), 'rdi');
	%save(sprintf('/user/chae2089/CWpaper/FN_REVISION/feature_for_2nd_model/eeg_combined_spo2_str/subject%d_rdi.mat',all(n1)), 'rdi');
