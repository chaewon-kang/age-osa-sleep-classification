# Sleep stage classification young model - SVM, kNN, MLP
...
for k in range(len(a)):
    ...
    for m in range(1, 6):
        ...
        # Y-Y: younger group validation set
        for n in test:
            test_data = loadmat("/user/chae2089/CWpaper/FN_REVISION/feature_for_sleep_stage/subject{}_PSD_TOTAL.mat".format(young[n]))
            test_data = test_data['TOTAL']
            test_set = np.append(test_set, test_data, 1)

            test_label = loadmat("/user/chae2089/CWpaper/FN_REVISION/feature_for_sleep_stage/subject{}_PSD_TOTAL_label.mat".format(young[n]))
        ...
        # Y-O: older group validation set
        ...
        # read train dataset
        for m in train:
            train_data = loadmat("/user/chae2089/CWpaper/FN_REVISION/feature_for_sleep_stage/subject{}_PSD_TOTAL.mat".format(young[m]))
            train_data = train_data['TOTAL']
            train_set = np.append(train_set, train_data, 1)

            train_label = loadmat("/user/chae2089/CWpaper/FN_REVISION/feature_for_sleep_stage/subject{}_PSD_TOTAL_label.mat".format(young[m]))
            train_label = train_label['label_TOTAL']
            train_set_label = np.append(train_set_label, train_label, 1)

        # adjust number of stage discrepencies for trainset
        ...
        # model prediction: Y-O
        fname_report = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/SVM/young/old_test/result_{}".format(num))
        fname_result = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/SVM/young/old_test/report_{}.csv".format(num))
        models.val_test(model_s, test_set, test_set_label, fname_result, fname_report)
        # model prediction: Y-Y
        fname_report = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/SVM/young/young_test/result_{}".format(num))
        fname_result = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/SVM/young/young_test/report_{}.csv".format(num))
        models.val_test(model_s, test_set_O, test_set_label_O, fname_result, fname_report)

        # kNN
        model_k = models.kNN_train(train_set2, train_set_label2)
        # model prediction: O-O
        fname_report = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/kNN/young/old_test/result_{}".format(num))
        fname_result = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/kNN/young/old_test/report_{}.csv".format(num))
        models.val_test(model_k, test_set, test_set_label, fname_result, fname_report)
        # model prediction: O-Y
        fname_report = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/kNN/young/young_test/result_{}".format(num))
        fname_result = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/kNN/young/young_test/report_{}.csv".format(num))
        models.val_test(model_k, test_set_O, test_set_label_O, fname_result, fname_report)

        # MLP
        model_m = models.SVM_train(train_set2, train_set_label2)
        # model prediction: O-O
        fname_report = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/MLP/young/old_test/result_{}".format(num))
        fname_result = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/MLP/young/old_test/report_{}.csv".format(num))
        models.val_test(model_m, test_set, test_set_label, fname_result, fname_report)
        # model prediction: O-Y
        fname_report = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/kNN/young/young_test/result_{}".format(num))
        fname_result = ("/user/chae2089/CWpaper/FN_REVISION/KFOLD_num_stage_fix/kNN/young/young_test/report_{}.csv".format(num))
        models.val_test(model_m, test_set_O, test_set_label_O, fname_result, fname_report)
