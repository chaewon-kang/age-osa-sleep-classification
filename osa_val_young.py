# OSA classification young model - SVM, kNN, MLP
    ...
    for m in range(1, 6):
        ...
        for n in test:
            ...
            if np.isin(normal_Y, young[n]).sum() == 1:
                ...
                test_set_Y = np.append(test_set_Y, test_data, 0)
                test_set_label_Y = np.append(test_set_label_Y, test_label, 0)
        ...
        test_set_label_Y = np.transpose(test_set_label_Y)
        ...
        for m in train:
            ...
            if np.isin(normal_Y, young[m]).sum() == 1:
                train_label = np.array([[1]])
                # rem & nrem features
                train_data = loadmat("/user/chae2089/CWpaper/FINAL/2nd_model/feature_nrem_rem_str/feature/subject{}".format(young[m]))
                train_data = np.transpose(train_data['nrem_rem'])
                # Train with SpO2
                #train_data = loadmat("/user/chae2089/CWpaper/FN_REVISION/feature_for_2nd_model/eeg_combined_spo2/subject{}".format(young[m]))
            ...
            # Including mtom class
            # elif np.isin(mtom_Y,young[m]).sum() == 1:
            ...
            elif np.isin(severe_Y, young[m]).sum() == 1:
                train_label = np.array([[2]])
                # rem & nrem
                train_data = loadmat("/user/chae2089/CWpaper/FINAL/2nd_model/feature_nrem_rem_str/feature/subject{}".format(young[m]))
                train_data = np.transpose(train_data['nrem_rem'])
